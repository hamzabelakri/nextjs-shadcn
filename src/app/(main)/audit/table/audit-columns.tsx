...
export const useAuditColumns = (): ColumnDef<AuditLog>[] => {
  ...
  return [
{
    ...
    cell: ({ row }) => {
      ...
      const formattedDate = date instanceof Date && !isNaN(date.getTime()) 
        ...
        : timestamp;
      return (
        <LongText className="max-w-36">{formattedDate}</LongText>
      );
    },
    meta: {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      // Hide compare button for non-update actions
      const hideCompare = action !== "update";
