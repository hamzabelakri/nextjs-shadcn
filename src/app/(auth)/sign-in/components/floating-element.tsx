...
// Component to solve hydration issues with random values
const DataPointsAnimation = () => {
  const [points, setPoints] = useState<Array<{top: string, left: string, delay: string, duration: string}>>([]);

  useEffect(() => {
    // Generate random points once on client-side
    const newPoints = Array(8).fill(0).map((_, i) => ({
...
const ParkingIllustration = () => {
  ...
  useEffect(() => {
    // Set initial position from client side
    setCarPosition({
