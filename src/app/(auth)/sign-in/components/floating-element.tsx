...
const ParkingIllustration = () => {
  ...
  useEffect(() => {
    // Animate car movement with deterministic positions
    ...
    const interval = setInterval(() => {
      setAnimationCounter(prev => {
        ...
        const position = predefinedPositions[newCounter % predefinedPositions.length];
    ...
    <div className="relative w-full max-w-lg aspect-video">
      <div className="absolute inset-0 bg-gradient-to-br from-blue-900 to-blue-700 rounded-2xl border border-blue-500/30 overflow-hidden shadow-2xl">
          ...
          {spots.map((spot) => (
            ...
            >
              ...
              <div className="absolute bottom-1 left-1 text-[10px] text-white/70">
                {t('parking_spot_p', { number: spot.id + 1 })}
              </div>
        ...
        <div className="absolute inset-0">
          {[...Array(8)].map((_, i) => {
            // Predefined positions for each data point to ensure consistency between SSR and client
            ...
            const animationDurations = [2.5, 3.2, 4.1, 3.8, 2.8, 3.5, 4.3, 3.1];
            
            const position = dataPointPositions[i];
            const duration = animationDurations[i];
